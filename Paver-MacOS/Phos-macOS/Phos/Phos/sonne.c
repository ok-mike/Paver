...
#define D               fs_RAM[sfp+0]      // D has register code 8
...
void xferboot( const char* fname)
{
    uint16_t pfsbuf[ 256];
    int addr = 0;
    int blk = 10;
    ...
    do {
        rd_blk_range( blk, 1, pfsbuf);
        cells = pfsbuf[ 5] / 2;
        for (int i=0; i<cells; i++) gp_ram_st( addr++, pfsbuf[ i+6], 0);
        blk = pfsbuf[ 4];
    } while (blk != 0);
...
//void xferboot( const char* fname)       // Read RAM from binary file
//{                                       // Network order
//
